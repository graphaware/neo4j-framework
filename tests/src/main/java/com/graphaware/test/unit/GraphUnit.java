...
public final class GraphUnit {
	...
	public static void assertNotEmpty(GraphDatabaseService database, InclusionPolicies inclusionPolicies) {
		...
		try (Transaction tx = database.beginTx()) {
			...
			nodeCount = database.getAllNodes()
					.stream()
					.filter(node -> inclusionPolicies.getNodeInclusionPolicy().include(node))
					.count();

			relCount = database.getAllRelationships()
					.stream()
					.filter(relationship -> inclusionPolicies.getRelationshipInclusionPolicy().include(relationship))
					.count();

			tx.success();
		}
		if (relCount == 0 && nodeCount == 0) {
			//TODO: Better toString on inclusion policies
